<script>
    ...
    export default {
      ...
      computed: {
        ...
        ...mapGetters("store", ["StorePdts"]),
        ...mapGetters("general", ["OtherBranches","BusinessSettings"]),
        ...mapGetters("instocktransfer", [
          "SaveStatus",
          "StockTransferCart",
        ]),
        ...
        rows() {
          return this.StockTransferCart ? this.StockTransferCart.length : 1;
        },
      ...
      mounted() {
        this.totalRows = this.StockTransferCart ? this.StockTransferCart.length : 1;
      },
      watch: {
        show(newValue) {
          if (newValue == true) {
      ...
      methods: {
        ...mapActions({
          FetchCart: "instocktransfer/fetchCart",
          FetchItems: "products/fetchProducts",
          FetchBranches: "general/fetchBranches",
          FetchSettings: "general/fetchBusinessSettings",
          AddCart: "instocktransfer/addToCart",
          DeleteCartItem: "instocktransfer/deleteCart",
          EditCartItems: "instocktransfer/editCart",
          SendItems: "instocktransfer/send",
          FetchStoreItems: "store/fetchStorePdts",
        }),
        ...
        checkItem() {
          if (this.data.product != "") {
            this.existingItem = this.StockTransferCart.filter((item) => {
        ...
        send() {
          let source = 0;
          if(this.StockTransferCart){
            this.StockTransferCart.forEach((item)=>{
              source = item.source_id;
            });
            this.totals.source = source;
          }                    
          if(this.totals.source == 0){
            this.totals.destination == 1;
          }else{
            this.totals.destination == 0;
          }
        ...
        focusProduct() {
          if(this.data.source == 0){
            this.$refs.cartProduct1.focus();
          }else{
            this.$refs.cartProduct2.focus();
          }      
        },
        nextPlease() {
    ...
    <template>
            ...
            <div class="card">
              <div class="card-body">
                ...
                <div class="table-responsive mb-0">
                  <b-table
                    class="datatables"
                    :items="StockTransferCart ? StockTransferCart : []"
                    :fields="fields"
                    responsive="sm"
                    :per-page="perPage"
      ...
      <Loader v-if="!StockTransferCart" />
                
                <b-form v-if="StockTransferCart?StockTransferCart.length > 0:false" class="mt-3">
                  <div class="row">                    
                    <div class="col-md-6">
                    ...
                    <div v-if="MyUserPerm.transfer_stock == 1" class="col-md-6 mt-4">
          ...
          <div class="col-md-4">
            <div class="card">
              <div class="card-body">
                <form action="">
                    <div class="form-group mb-3" v-if="StockTransferCart?StockTransferCart.length==0:false">
                        <label>Source - Destination<span class="text-danger">*</span></label>
                        <div class="input-group">
                        <select class="form-control" v-model="data.source">
                            <option value="0" v-if="BusinessSettings?BusinessSettings.store==1:false">Shop to Store</option>
                            <option value="1" v-if="BusinessSettings?BusinessSettings.store==1:false">Store to Shop</option>
                        </select>
                        </div>
                    </div>
                    <div class="form-group mb-3" v-if="data.source == 0">
                    ...
                    <div class="form-group mb-3" v-if="data.source == 1">
                      <label>Product <span class="text-danger">*</span></label>
                      <input
                        class="form-control"
                        v-model="data.product"
