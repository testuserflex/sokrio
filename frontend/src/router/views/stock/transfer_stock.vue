...
export default {
  ...
  methods: {
    ...mapActions({
      FetchCart: "stocktransfer/fetchCart",
      FetchItems: "products/fetchProducts",
      FetchBranches: "general/fetchBranches",
      FetchSettings: "general/fetchBusinessSettings",
      AddCart: "stocktransfer/addToCart",
      DeleteCartItem: "stocktransfer/deleteCart",
      EditCartItems: "stocktransfer/editCart",
      SendItems: "stocktransfer/send",
    ...
    send() {
      this.totals.source = this.data.source;
      if (this.totals.destination == "") {
...
<template>
  <Layout>
    <div class="row py-3 text-center">
      <h4 class="text-primary">{{business}}</h4><h4 v-if="GeneralSettings.show_branchname == 1 && branch"> - {{branch}}</h4>
    </div>
        ...
        <div class="card">
          ...
          <Loader v-if="!StockTransferCart" />            
            <b-form v-if="StockTransferCart?StockTransferCart.length > 0:false" class="mt-3">
              <div class="row">
                <h3>You are transfering items from <span v-if="data.source==0">Shop</span><span v-else>Store</span></h3>
                <div class="col-md-4">
                  <div class="form-group">
                    <label>Destination Branch <span class="text-danger">*</span></label>
                    <model-list-select
                      :list="OtherBranches ? OtherBranches : []"
                      v-model="totals.destination"
                      option-value="id"
                      option-text="name"
