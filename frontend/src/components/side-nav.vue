...
export default {
    ...
    //     MyUserPerm(newValue){
    //         if(newValue){
    //             let xx = this.$route.path;
    //             if(xx == "/default/dash"){
    //             router.push('/');
                    
    //             }else{
    //             router.push('/default/dash');
...
<div id="sidebar-menu">
    ...
    <ul id="side-menu" class="metismenu list-unstyled">
        <template v-for="item in menuItems">
            <div v-if="!item.actionpage || item.actionpage && BranchData.BranchId != 0" :key="item.id" >
                <li class="menu-title" v-if="item.isTitle" :key="item.id">
                    {{ $t(item.label) }}
                </li>
                <li v-if="!item.isTitle && !item.isLayout && !item.perm == '' && MyUserPerm[item.perm] == 1" :key="item.id">
                    ...
                    <ul v-if="hasItems(item)" class="sub-menu" aria-expanded="false">                        
                        <li v-for="(subitem, index) of item.subItems" :key="index">
                            <div v-if="!subitem.actionpage || subitem.actionpage && BranchData.BranchId != 0" :key="index" >
                        <!-- <router-link :to="subitem.link" v-if="!hasItems(subitem)" class="side-nav-link-ref">{{ $t(subitem.label) }}</router-link> -->
                            <router-link :to="subitem.link" v-if="!hasItems(subitem) && !subitem.perm == '' && MyUserPerm[subitem.perm] == 1 " class="side-nav-link-ref">{{ $t(subitem.label) }}</router-link> 
                            <a v-if="hasItems(subitem)" class="side-nav-link-a-ref has-arrow" href="javascript:void(0);">{{ $t(subitem.label) }}</a>
                            <ul v-if="hasItems(subitem)" class="sub-menu mm-collapse" aria-expanded="true">
